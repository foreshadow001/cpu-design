...
本实验要求同学们基于miniRV指令集实现单周期CPU。miniRV是RV32I的子集，因而兼容RISC-V。

miniRV指令集共包含32个通用寄存器和**44条指令**（8条示例 + 36条待实现），按指令格式分为R型17条、I型15条、S型3条、B型6条、U型2条、J型1条。
...
> **分组**：浅蓝色 = A组 (18条)，浅橙色 = B组 (18条)，绿色 = demo已实现 (8条: `addi, ori, slli, lui, lw, beq, bne, jal`)。A组和B组各含18条，组内成员分别负责一组。

## 1. miniRV通用寄存器

miniRV具有32个32位通用寄存器，与RV32I完全相同，如表2-1所示。
...
在汇编语言中，既可以通过寄存器号也可以通过助记符来引用一个寄存器。例如，汇编中的"x0"和"zero"均表示第0个寄存器；"x8"、"s0"和"fp"均表示第8个寄存器。

## 2. miniRV指令格式

miniRV指令集的指令格式与RISC-V完全相同，如图2-1所示。

[图2-1 miniRV指令格式 —— R型、I型、S型、B型、U型、J型六种指令格式]

## 3. miniRV指令速查表

miniRV指令的格式、功能、语法及解释汇总如表2-2所示。其中，浅蓝色背景的是A组指令，浅橙色的则是B组指令，而绿色字体的指令则是demo工程已经实现的指令。

[表2-2 miniRV指令速查表]

下面对每条指令的格式、功能等给出详细解释，所使用的符号约定如表2-3所示。
...
- 指令名：乘法取低32位
- 使用语法：`mul rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`相乘，乘积的低32位写入`rd`。
- 汇编例子：
  ```assembly
  mul s5, t1, s1    # (s5) ← ((t1) × (s1))[31:0], (PC) ← (PC) + 4
  ```

#### mulh

- 指令名：有符号乘法取高32位
- 使用语法：`mulh rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`作有符号乘法，乘积的高32位写入`rd`。
- 汇编例子：
  ```assembly
  mulh s5, t1, s1    # (s5) ← ((t1) × (s1))[63:32], (PC) ← (PC) + 4
  ```

#### mulhu

- 指令名：无符号乘法取高32位
- 使用语法：`mulhu rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`作无符号乘法，乘积的高32位写入`rd`。
- 汇编例子：
  ```assembly
  mulhu s5, t1, s1    # (s5) ← ((t1) × (s1))[63:32], (PC) ← (PC) + 4
  ```
...
- 指令名：有符号除法
- 使用语法：`div rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`视为有符号数，并计算`rs1` ÷ `rs2`，商写入`rd`。
- 汇编例子：
  ```assembly
  div s5, t1, s1    # (s5) ← (t1) / (s1), (PC) ← (PC) + 4
  ```

#### divu

- 指令名：无符号除法
- 使用语法：`divu rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`视为无符号数，并计算`rs1` ÷ `rs2`，商写入`rd`。
- 汇编例子：
  ```assembly
  divu s5, t1, s1    # (s5) ← (t1) / (s1), (PC) ← (PC) + 4
  ```
...
- 指令名：有符号求余
- 使用语法：`rem rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`视为有符号数，并计算`rs1` ÷ `rs2`，余数写入`rd`。
- 汇编例子：
  ```assembly
  rem s5, t1, s1    # (s5) ← (t1) % (s1), (PC) ← (PC) + 4
  ```

#### remu

- 指令名：无符号求余
- 使用语法：`remu rd, rs1, rs2`
- 功能描述：把寄存器`rs1`和寄存器`rs2`视为无符号数，并计算`rs1` ÷ `rs2`，余数写入`rd`。
- 汇编例子：
  ```assembly
  remu s5, t1, s1    # (s5) ← (t1) % (s1), (PC) ← (PC) + 4
  ```
